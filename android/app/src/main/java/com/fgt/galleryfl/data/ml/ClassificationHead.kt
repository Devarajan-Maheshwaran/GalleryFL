...
class ClassificationHead(private val numClasses: Int = 20) {
    ...
    fun forward(features: FloatArray): FloatArray {
        ...
        for (j in 0 until numClasses) {
            ...
            z2[j] = sum
            output[j] = 1f / (1f + exp(-sum)) // Sigmoid
            a2[j] = output[j]
        }
    ...
    fun backward(targets: FloatArray): Gradients {
        // Assume BCE loss. dL/dz2 for BCE + Sigmoid = (a2 - y)
